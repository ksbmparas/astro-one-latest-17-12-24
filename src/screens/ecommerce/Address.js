...
const Address = ({  dispatch, addressData }) => {
    ...
    const renderAddressItem = ({ item }) => (
        <View style={{ paddingHorizontal: 10 }}>
            ...
            >
                <View style={{ position: "absolute", right: 10, gap:10,}}>
                    <TouchableOpacity
                        onPress={() => deleteAddress(item?._id)}
                        style={{ padding: 5, }}
                    >
                        <AntDesign name="delete" size={30} color="#FE6006" />
                    </TouchableOpacity>

                    <TouchableOpacity
                        onPress={() => navigation.navigate('updateaddress', { addressId: item })}
                        style={{ padding: 5, }}
                    >
                        <AntDesign name="edit" size={30} color="#FE6006" />
                    </TouchableOpacity>
                </View>
                ...
                <Text style={styles.addressDetails}>
                    Address: {item.house}, {item.area}, {item.city}, {item.state}, {item.pincode}
                </Text>
            </TouchableOpacity>
        </View>
    );
